...
// Source: modal.js
    angular.module('mgcrea.ngStrap.modal', ['mgcrea.ngStrap.helpers.dimensions'])
        ...
        .provider('$modal', function () {
            ...
            var defaults = this.defaults = {
                animation: 'am-fade',
                backdropAnimation: 'am-fade-backdrop',
                prefixClass: 'modal',
                prefixEvent: 'modal',
                placement: 'top',
                ...
                function ModalFactory(config) {
                    ...
                    scope.$hide = function () {
                        ...
                        if(options.cancelCallback)
                            options.cancelCallback(options.element,msg,scope);
                        else if(scope.cancelCallback)
                            scope.cancelCallback(options.element,msg,scope);
                    };
                    scope.$show = function () {
                        scope.$$postDigest(function () {
                            $modal.show();
                    ...
                    scope.$modalButtonClick = function (msg) {
                        var returnFlag = true;
                        if(options.callback)
                        returnFlag = options.callback(options.element,msg,scope);
                        else if(scope.callback)
                        returnFlag = scope.callback(options.element,msg,scope);
                    ...
                    $modal.show = function () {
                        ...
                        if(parent.hasClass('alert-container')){
            ...
            return {
                ...
                link: function postLink(scope, element, attr, transclusion) {
                    ...
                    attr.bsModal && scope.$watch(attr.bsModal, function (newValue, oldValue) {
                        ...
                        angular.forEach(['title', 'content'], function (key) {
                            scope[key] = $sce.trustAsHtml(scope[key]);
                        });
                    }, true);
        ...
        .directive('bsPopover', ["$window", "$sce", "$popover", function ($window, $sce, $popover) {
            ...
            return {
                ...
                link: function postLink(scope, element, attr) {
                    ...
                    // Directive options
                    var options = {scope: scope};
                    angular.forEach(['template', 'contentTemplate', 'placement', 'container', 'target', 'delay', 'trigger', 'keyboard', 'html', 'animation', 'customClass','hoverHold','autoPlace'], function (key) {
                        if (angular.isDefined(attr[key])) options[key] = attr[key];
                    });
        ...
        .provider('$tooltip', function () {
            ...
            this.$get = ["$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "dimensions", "$$rAF", function ($window, $rootScope, $compile, $q, $templateCache, $http, $animate, dimensions, $$rAF) {
                ...
                function TooltipFactory(element, config) {
                    ...
                    if(options.cardData){
                        $tooltip.$scope.cardData = options.cardData;
                    }
                            ...
                            return fetchTemplate(options.contentTemplate)
                                .then(function (contentTemplate) {
                                    if(options.preProcessHtml){
                                        contentTemplate = options.preProcessHtml(contentTemplate);
                                    }
                    ...
                    $tooltip.init = function () {
                        ...
                        angular.forEach(triggers, function (trigger) {
                            if (trigger === 'click') {
                                ...
                                if(trigger === 'hover'&&options.moveWithMouse){
                                    element.on('mousemove',$tooltip.$onMouseMove);
                                }
                    ...
                    $tooltip.destroy = function () {
                        ...
                        for (var i = triggers.length; i--;) {
                            ...
                            if (trigger === 'click') {
                                ...
                                if(trigger === 'hover'&& options.moveWithMouse)
                                	element.off('mousemove',$tooltip.$onMouseMove)
                    ...
                    $tooltip.suspendRes = false;

                    $tooltip.$onMouseMove = function(evt){

                        if(!$tooltip.suspendRes){
                            var tipHeight = tipElement.prop('offsetHeight');
                            var tipWidth = tipElement.prop('offsetWidth');
                            if((evt.clientX+tipWidth)>document.body.clientWidth){
                                tipElement.css({top:evt.clientY,left:evt.clientX-tipWidth});
                            }else{
                                tipElement.css({top:evt.clientY,left:evt.clientX});
                            }
                            $tooltip.suspendRes = true;
                            setTimeout(function(){
                                $tooltip.suspendRes= false;
                            },100)
                        }
                    ...
                    $tooltip.leave = function () {
                        ...
                        if(options.trigger == "hover")
                            $tooltip.$element&&$tooltip.$element.unbind().mousedown(options.hoverHold? $.noop:$tooltip.hide).mouseenter(function(){
                            clearTimeout(timeout);
                    ...
                    $tooltip.$applyPlacement = function () {
                        ...
                        if(options.trigger === "hover"&&options.mouseWithMouse){
                            return;
                        }
                        ...
                        var elementSize = {width:element.outerWidth(),height:element.outerHeight()};
                        ...
                        var arrowPosition = getArrowOffset(options.placement, elementPosition, tipWidth, tipHeight);
                        arrowPosition.top += 'px';
                        arrowPosition.left += 'px';
                        angular.element(".arrow",tipElement).css(arrowPosition);
                    };
                    ...
                    function getArrowOffset(placement, position, actualWidth, actualHeight){
                        var offset;
                    ...
                    function getCalculatedOffset(placement, position, actualWidth, actualHeight) {
                        ...
                        var topMax = position.top + position.height / 2 + actualHeight / 2;
                        var topMin = position.top + position.height / 2 - actualHeight / 2;
                        var top = topMin <0? 0 :topMax > clientHeight ? (clientHeight - actualHeight):(position.top + position.height / 2 - actualHeight / 2);
                        var calcLeft = position.left + position.width / 2 - actualWidth / 2;
                        var actualLeft = calcLeft>0?calcLeft<(clientWidth-actualWidth)?calcLeft:(clientWidth-actualWidth):0;
                        switch (split[0]) {
                            case 'right':
                                offset = {
                                    top: top,
                        ...
                        if (split[0] === 'top' || split[0] === 'bottom') {
                            switch (split[1]) {
                                case 'left':
                                    ...
                                    break;
                                case 'right':
                                    offset.left = calcLeft+actualWidth/2<(clientWidth-actualWidth)?actualLeft+actualWidth/2:actualLeft;
                            }
                        } else if (split[0] === 'left' || split[0] === 'right') {
                            switch (split[1]) {
                                case 'top':
        ...
        .directive('bsTooltip', ["$window", "$location", "$sce", "$tooltip", "$$rAF", function ($window, $location, $sce, $tooltip, $$rAF) {
            ...
            return {
                ...
                link: function postLink(scope, element, attr, transclusion) {
                    ...
                    // Directive options
                    var options = {scope: scope};
                    angular.forEach(['template', 'contentTemplate', 'placement', 'container', 'target', 'delay', 'trigger', 'keyboard', 'html', 'animation', 'type', 'customClass','hoverHold'], function (key) {
                        if (angular.isDefined(attr[key])) options[key] = attr[key];
                    });
